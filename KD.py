# -*- coding: utf-8 -*-
"""
Created on Tue Feb 25 10:37:58 2020
...
df = df[df['日期']> datetime.date(2018,12,31) ]
...
label=list()
label.append('H')
for i in range(len(df['K(9)'])-2,-1,-1):
    if (df['K(9)'][i]>df['D(9)'][i]) & (df['K(9)'][i+1]<df['D(9)'][i+1]) :
        label.append('B')
    elif (df['K(9)'][i]<df['D(9)'][i]) & (df['K(9)'][i+1]>df['D(9)'][i+1]) :
        label.append('S')
    else:
        label.append('H')
...
ax2.plot(weekday,df['K(9)'],label="K9",color = 'steelblue' )
ax2.plot(weekday,df['D(9)'],label="D9",color = 'g' )
...
cost=[0]*(len(df['K(9)'])+1)
profit=[0]*(len(df['K(9)'])+1)

for i in range(len(df['K(9)'])-1,-1,-1):
